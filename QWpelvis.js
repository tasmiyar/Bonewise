...
const targetMeshNames = [
    ...
    'lunate_surface',
    'obturator_crest',
    'outer_lip_of_iliac_crest',
    'pecten_pubis',
    'posterior_gluteal_line',
    'posterior_inferior_iliac_spine',
    'posteior_superior_iliac_spine',
    'pubic_symphysis',
    'pubic_tubercle',
    'supra-acetabular_groove',
    'tuberculum_of_iliac_crest',

].map(name => name.replace(/_/g, ' ')); // Normalize names by replacing underscores with spaces

// Load the femur GLTF model
const loader = new GLTFLoader();
loader.setPath('./public/lower/'); // Ensure the path starts with './'
loader.load('pelvis.gltf', (gltf) => {
    const model = gltf.scene;

    // Rotate the model 180 degrees around the X axis to flip it upright
