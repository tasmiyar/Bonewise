...
loader.load('tibia.gltf', (gltf) => {
    ...
    // --- Scale the model to match femur size ---
    const size = box.getSize(new THREE.Vector3());
    const desiredHeight = 5.5; // Adjust this value to match your femur's visual height
    const scale = desiredHeight / size.y;
    modelGroup.scale.set(scale, scale, scale);

    // --- Move the camera further back to zoom out ---
    camera.position.set(0, 2, 8); // Further back and slightly higher
...
camera.position.set(0, 2, 8); // Move camera further back and slightly higher
...
window.addEventListener('click', (event) => {
    ...
    if (intersects.length > 0) {
        ...
        console.log("Clicked mesh raw name:", clickedMesh.name); // ← ADD THIS
    
        const meshName = clickedMesh.name.replace(/_/g, ' ').replace(/\d+/g, '') || 'Unnamed Mesh';
    
        meshNameBox.style.opacity = '0';
        setTimeout(() => {
            meshNameBox.textContent = `${meshName}`;
            meshNameBox.style.opacity = '1';
        }, 200);
    }
});
